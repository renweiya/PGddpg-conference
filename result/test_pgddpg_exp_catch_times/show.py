...
flags.DEFINE_string('env_name', 'round_up_reward_shaping', 'env used')
...
flags.DEFINE_string('prey_model_path', './exp_result/3v1/saved_models/round_up_reward_shaping_5/seed_ddpg/20200908090736/model-44000',
                    'path of the pre-trained prey model')
flags.DEFINE_bool('reload_predator', True, '')
flags.DEFINE_string('predator_model_path',"exp_result/prey/model-prey-08" ,#'./exp_result/3v1/saved_models/round_up_reward_shaping_10/seed_maddpg/20200908091048/model-40000',
...
flags.DEFINE_string('predator_policy', 'ddpg', 'predator_policy')
flags.DEFINE_float('rlpl_beta', 0.5, 'rlpl_beta') #jone   # 0 < rlpl_beta < 1 , if rlpl_beta < 0 pgddpg rlpl_beta_dec
flags.DEFINE_string('prey_policy', 'fixpolicy3', 'prey_policy: [random, fixed, ddpg , fixpolicy]')
flags.DEFINE_integer('episodes', 200000, 'maximum training episode')
...
flags.DEFINE_integer('min_buffer_size', 30000, 'minimum buffer size before training')#30000
...
def train():
    ...
    with tf_utils.make_session().as_default() as sess:
        ...
        saver = tf.train.Saver(max_to_keep=int(FLAGS.episodes / (2*FLAGS.save_checkpoint_every_epoch))) #modify by laker
...
def save_model(saver, sess, episode):
    model_path = FLAGS.model_save_dir.format(FLAGS.env_name, FLAGS.exp_name, FLAGS.seed)#modify by laker
