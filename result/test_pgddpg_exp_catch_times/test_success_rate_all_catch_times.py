...
flags.DEFINE_string('env_name', 'round_up_count_co_times', 'env used')
...
# test for all model 
predator_model_path = [
                        # "test_model/predator/ddpg_rs_5/model-40000",
...
flags.DEFINE_integer('max_episode_len', 2000, 'maximum step of each episode')#modify 60
...
def make_env(scenario_name, max_step_before_punishment,predator_policy):
    ...
    env = MultiAgentEnv(world, scenario.reset_world, scenario.reward, scenario.observation,
                        ...
                        other_callbacks=scenario.reset_prey)
    return env
...
def train():
    for i in range(len(predator_model_path)):
        ...
        with tf_utils.make_session().as_default() as sess:
            ...
            while len(episode_rewards) <= FLAGS.episodes:#100
                ...
                if  len(episode_rewards) % show_fre == 0: # modify
                    if terminal==True: 
                        length_k=show_fre
                        if len(catch_times)>length_k:
                            # print("sucess rate",sum(catch_times[-length_k:])/len(catch_times[-length_k:]))
                            print("sucess rate", catch_times[0])
        ...
        # do record
        log_path = './exp_result/evaluate_record_obstacle_co_times'
        if not os.path.exists(log_path):
            os.makedirs(log_path, exist_ok=True)
        losses_transformation_file_name = log_path + '/' + pa_model_path.split("/")[2] + '_cooperate_times_in_2000_steps.txt'
        with open(losses_transformation_file_name, 'a') as fp:
            fp.write(pa_model_path.split("/")[2] +"\t\t"+ pe_model_path.split("/")[2]  +"\t\t"+ str(sum(catch_times)/len(catch_times))+"\t\t"+ str(max(catch_times))+"\n") #+"\t\t"+ str(np.mean(episode_step_r))+"\n")#predator prey success_rate episode_step
        print("-----------***_cooperate_times_in_2000_steps**********-------------")
        print(pa_model_path.split("/")[2] +"\t\t"+ pe_model_path.split("/")[2]  +"\t\t"+ str(sum(catch_times)/len(catch_times))+"\t\t"+ str(max(catch_times)))
        print("-----------******-*****-*****-*****-*****-*****-*****-------------")
# for debug below ..........................................................
