...
import datetime
DATETIME = "pre_trained_prey_"+datetime.datetime.now().strftime('%Y%m%d%H%M%S')
...
flags.DEFINE_string('predator_model_path', '. ',
                    'path of the pre-trained predator model')

# Training parameters
flags.DEFINE_bool('learning', True, 'train the agents')
flags.DEFINE_string('exp_name', '3v1_', 'exp_name')
flags.DEFINE_string('predator_policy', 'pgddpg', 'predator_policy: [pgddpg, ddpg]') 
flags.DEFINE_float('rlpl_beta', 0.8, 'rlpl_beta') #jone   # 0 < rlpl_beta < 1 , if rlpl_beta < 0 pgddpg rlpl_beta_dec
...
if FLAGS.predator_policy[0:6] =="pgddpg" or FLAGS.predator_policy[0:8] =="pgmaddpg":
    ...
    suffix = ''
prey_name  =  os.path.splitext(os.path.basename(FLAGS.prey_model_path))[0]
flags.DEFINE_string('model_save_dir', './exp_result/{}/saved_models/{}/seed_{}/{}/model'.format(FLAGS.exp_name,prey_name, FLAGS.predator_policy+suffix,DATETIME),
                    'Model saving dir')
flags.DEFINE_string('learning_curve_dir', './exp_result/{}/learning_curves/{}/seed_{}/{}'.format(FLAGS.exp_name,prey_name, FLAGS.predator_policy+suffix,DATETIME),
                    'learning_curve_dir')
flags.DEFINE_string('tensorboard_dir', './exp_result/{}/tensorboard_dir/{}/seed_{}/{}'.format(FLAGS.exp_name,prey_name, FLAGS.predator_policy+suffix,DATETIME),
                    'tensorboard_dir')
FLAGS = flags.FLAGS  # alias
# init tf summaries
summary_path = FLAGS.learning_curve_dir
# init tf summaries
tensorboard_save_dir = FLAGS.tensorboard_dir
# init tf model_path
model_path = FLAGS.model_save_dir
print('||||||||||||||||||--------Log Path------||||||||||||||||||')
print('summary_path', summary_path)
print('tensorboard_dir', tensorboard_save_dir)
print('model_path', model_path)
print('||||||||||||||||||--------Log Path------||||||||||||||||||')
#save presetting_parameters
if not os.path.exists(summary_path):
    os.makedirs(summary_path, exist_ok=True)
presetting_parameters_log_file_name = summary_path + '/' + prey_name + '_presetting_parameters_log.txt' 
    ...
    with tf_utils.make_session().as_default() as sess:
        ...
        while len(episode_rewards) <= FLAGS.episodes:#140000
            ...
            if 1:#FLAGS.train_prey:
                ...
                # action_2_dim_n = [agent.get_actions(observations=[obs], single=True, step_explore=step_explore_, explore_direction=0.0,gama_gussian=gama_gussian_) for agent, obs in zip(agents, obs_n)]
            ...
            if FLAGS.learning:
                ...
                if ended:
                    ...
                    #jone
                    train_episode_summary.value.add(simple_value=sum(sucess_rate[-200:])/len(sucess_rate[-200:]), tag="success_rate")
                    tensorboard_writer.add_summary(train_episode_summary, len(episode_rewards))
                    tensorboard_writer.flush()
...
def save_model(saver, sess, episode):
    model_path = FLAGS.model_save_dir#modify by laker
...
def record_logs(**kwargs):
    ...
    rew_file_name = log_path + '/' + prey_name + '_rewards.pkl'
    with open(rew_file_name, 'wb') as fp:
        pickle.dump(kwargs['agent_rewards'], fp)
        
    rate_file_name = log_path + '/' + prey_name + '_sucess_record.pkl'
    with open(rate_file_name, 'wb') as fp:
        pickle.dump(kwargs['sucess_record'], fp)


    buffer_mean = log_path + '/' + prey_name + '_buffer.pkl'
