...
def p_train(make_obs_ph, act_space, p_index, p_func, q_func,plan_func, optimizer, grad_norm_clipping=None, local_q_func=True,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        train = U.function(inputs=[train_obs_input] + [train_action_input] + [weight_ph,n_steps_annealings,rlpl_beta,],
...
class AgentTrainer_PGDDPG(AgentTrainer):
    ...
    def __init__(self, name, learn_model,plan_model, obs_shape, act_space, agent_index, buffer,
        ...
        self.start_beta_decay=0
        self.rlpl_beta_show= FLAGS.rlpl_beta
    ...
    def do_training(self, agent, train_step, end):
        ...
        if train_step % 20000==0:
            print("self.rlpl_beta_show",self.rlpl_beta_show)
    ...
    def update(self, agent):
        ...
        self.start_beta_decay=self.start_beta_decay+1


        # rlpl_beta=1-pow(0.995,0.01*self.start_beta_decay)#4w
        # rlpl_beta=1-pow(0.95,0.01*self.start_beta_decay)#4q
        ## if rlpl_beta>0.95:
        ##     rlpl_beta=0.95

        rlpl_beta= FLAGS.rlpl_beta # 0.9-0.6
        self.rlpl_beta_show=rlpl_beta
        # print("rlpl_beta",rlpl_beta)
        
        p_loss,xxx,yyy,zzz= self.p_train(*([obs] + [act] + [np.ones_like(target_q),self.n_steps_annealing,rlpl_beta,]))
        # print('xxx:',xxx)
        # print('yyy:',yyy)
        # print('zzz:',zzz)  
