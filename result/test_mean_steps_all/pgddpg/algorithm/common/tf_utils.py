...
def initialize(sess=None):
    ...
    for var in tf.global_variables():
        try:
            ...
            # print('uninitialized variables: ', var)
            uninit_vars.append(var)

    if len(uninit_vars) == 0:
        print("None variables need to be initialized...")
    else:
        print('Variables to be initialized: ')
        print(uninit_vars)
...
def save_state(fname, saver=None, global_step=0):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.check_nan:
            if any(np.isnan(r).any() for r in results):
                ...
                vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="agent_0/p_func")
                print('variables: ================== \n')
                for var in vars:
                    print(var.name, ": ", get_session().run(var))
