...
def compute_magnitude_angle(target_location, current_location, orientation):
  ...
  forward_vector = np.array([math.cos(math.radians(orientation)), math.sin(math.radians(orientation))])
  if np.dot(forward_vector, target_vector) < 0: d_angle = -d_angle

  return (norm_target, d_angle)

def compute_the_delta_angle(target_vector, ego_yaw):#laker
  """
  Compute relative angle and distance between a target_location and a current_location

  :param target_vector: target-location
  :param turn_steer: expect turn_steer
  :return: a tuple composed by the distance to the object and the angle between both objects
  """
  norm_target = np.linalg.norm(target_vector)
      
  forward_vector = np.array([math.cos(ego_yaw), math.sin(ego_yaw)])
  # print("target_location:",target_location.x,target_location.y)
  ...
  # 
  if norm_target: # not equal to zero
    d_angle = math.acos(np.dot(forward_vector, target_vector)/ norm_target)#math.degrees(math.acos(np.dot(forward_vector, target_vector) / norm_target))
  else: #0/0 protect
    d_angle=0 
  # print("d_angle:",d_angle)  
  #rotate to decide the sign.
  ego_yaw_temp = ego_yaw+np.pi/2
  forward_vector = np.array([math.cos(ego_yaw_temp), math.sin(ego_yaw_temp)])
  if np.dot(forward_vector, target_vector) < 0: 
    d_angle = -d_angle
  return d_angle
  #
  # print("d_angle-turn_steer,d_angle,turn_steer",d_angle-turn_steer,d_angle,turn_steer)
  # print("######################################compute_delta_angle######################################")
  # delta_angle=d_angle-turn_steer
  # # print("before_delta_angle:",delta_angle)
  # #-pi<delta_angle<pi
  # while delta_angle>=np.pi:
  #   delta_angle=delta_angle-2*np.pi
  # while delta_angle<-np.pi:
  #   delta_angle=delta_angle+2*np.pi
  # # print("after_delta_angle:",delta_angle)
  # return (d_angle-turn_steer,d_angle,turn_steer)


def compute_angle_error(curr_course, e_course):
  #角度转换
  curr_vector = np.array([math.cos(math.radians(curr_course)), math.sin(math.radians(curr_course))])
  exp_vector = np.array([math.cos(math.radians(e_course)), math.sin(math.radians(e_course))])
  angle_error = math.degrees(math.acos(np.dot(curr_vector, exp_vector) ))
  #判断一下角度的+-
  orientation = curr_course+90
  forward_vector = np.array([math.cos(math.radians(orientation)), math.sin(math.radians(orientation))])
  if np.dot(forward_vector, exp_vector) < 0: angle_error = -angle_error
...
def distance_v2v(vehicle1, vehicle2):
...
def rgb_to_display_surface(rgb, display_size_x = 255, display_size_y = None):
  """
  ...
  """
  flag = False#图像数据整数的话会处理成0-1之间的浮点数 jone
  if display_size_y is None:
    display_size_y=display_size_x
  else:
    flag = True   
  surface = pygame.Surface((display_size_y, display_size_x)).convert()
  display = skimage.transform.resize(rgb, (display_size_x, display_size_y))
  display = np.flip(display, axis=1)
  display = np.rot90(display, 1)

  if flag :display = display*255  #把它变回来  jone
