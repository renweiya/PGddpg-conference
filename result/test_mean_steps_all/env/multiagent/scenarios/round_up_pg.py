    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            if not other.adversary:  # 我的观察里有被捕食者，添加其速度
                #for sure
                # rand_pos= -1 + 2*np.random.random(2) #(-1,1)
                # print("rand_pos",rand_pos)
                
                other_vel.append(other.state.p_vel)
                
                # print("force_order_norm",force_order_norm)
                prey_estimate_vel=other.state.p_vel


                # prey_v_norm = np.sqrt(np.square(prey_estimate_vel[0]) + np.square(prey_estimate_vel[1]))

                # v_order=(other.state.p_pos - agent.state.p_pos)+prey_estimate_vel-agent.state.p_vel #expect velocity
                
                ### method 1    
                #f=ma,agent.mass=1
                s_order=other.state.p_pos- agent.state.p_pos+(prey_estimate_vel-agent.state.p_vel)*world.dt                  
                attractive_force_order=2*s_order/(world.dt*world.dt)
                ### method 2  # same attractive_force_order, but 1 better than 2 in  append(s_order)  
                # s_order=other.state.p_pos- agent.state.p_pos+prey_estimate_vel*world.dt  #expect s
                # attractive_force_order=2*(s_order-agent.state.p_vel*world.dt)/(world.dt*world.dt)
                ...
                #method 2-?
                # apf_s_order=other.state.p_pos - agent.state.p_pos
                # if np.max(np.fabs(apf_s_order)) > 0.7071*agent.max_speed*world.dt:#the maximum distance agent can move
                #     apf_s_order = (apf_s_order/np.max(np.fabs(apf_s_order)))*0.7071*agent.max_speed*world.dt
                # s_real_order=apf_s_order+(prey_estimate_vel-agent.state.p_vel)*world.dt  
                # attractive_force_order=2*s_real_order/(world.dt*world.dt)
                # force_order=attractive_force_order+0*repulsive_force
                # if np.max(np.fabs(force_order)) > 1:#force, force_x belong [-1,1],force_y belong [-1,1]
                #     force_order = force_order/np.max(np.fabs(force_order))
