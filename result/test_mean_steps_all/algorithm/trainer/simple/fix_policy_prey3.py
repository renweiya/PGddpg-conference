...
class FixPolicyPrey3():
    ...
    def get_actions(self, observations, single=True, step_explore=0, explore_direction=0,gama_gussian=0):
        ...
        if dist_ > 0.4:
            ...
            e_force = 0.0
        a_force = np.array(agent3_v)/d_v3*np.clip((-np.log10(d_v3)),0,100) + np.array(agent2_v)/d_v2*np.clip((-np.log10(d_v2)),0,100)  + np.array(agent1_v)/d_v1*np.clip((-np.log10(d_v1)),0,100) 
