...
seed = np.array([[pgddpg_vs_prey00[i],pgddpg_vs_prey01[i],pgddpg_vs_prey02[i],pgddpg_vs_prey03[i],pgddpg_vs_prey04[i],pgddpg_vs_prey05[i],pgddpg_vs_prey06[i],pgddpg_vs_prey07[i],pgddpg_vs_prey08[i],pgddpg_vs_prey09[i]] for i in range(start,end) ])
seed_max = np.max(seed,1)
seed_min = np.min(seed,1)
seed_mean = np.mean(seed,1) 
plt.plot(zz, seed_mean, label='ddpg_seed', linewidth=1,color='r' )
plt.fill_between(zz, seed_min, seed_max,color='r',alpha = 0.1)

seed = np.array([[ddpg_vs_prey00[i],ddpg_vs_prey01[i],ddpg_vs_prey02[i],ddpg_vs_prey03[i],ddpg_vs_prey04[i],ddpg_vs_prey05[i],ddpg_vs_prey06[i],ddpg_vs_prey07[i],ddpg_vs_prey08[i],ddpg_vs_prey09[i]] for i in range(start,end) ])
seed_max = np.max(seed,1)
seed_min = np.min(seed,1)
seed_mean = np.mean(seed,1) 
...
seed = np.array([[maddpg_vs_prey00[i],maddpg_vs_prey01[i],maddpg_vs_prey02[i],maddpg_vs_prey03[i],maddpg_vs_prey04[i],maddpg_vs_prey05[i],maddpg_vs_prey06[i],maddpg_vs_prey07[i],maddpg_vs_prey08[i],maddpg_vs_prey09[i]] for i in range(start,end) ])
seed_max = np.max(seed,1)
seed_min = np.min(seed,1)
seed_mean = np.mean(seed,1) 
plt.plot(zz, seed_mean, label='ddpg_seed', linewidth=1,color='g' )
...
plt.ylabel('Avg_Success_Rate',font2)
# plt.legend(loc = "lower right",labels =[r"pgddpg($\beta=0.8$) vs all 10 preys",r"ddpg($\alpha=1$) vs all 10 preys",r"maddpg($\alpha=5$) vs all 10 preys","ddpg vs all 10 preys","maddpg vs all 10 preys"],fontsize = 25)
# plt.legend(loc = "lower right",labels =[r"PGDDPG ($\beta=0.8$) vs. all 10 prey",r"DDPG ($\alpha=1$) vs. all 10 prey",r"MADDPG ($\alpha=5$) vs. all 10 prey",r"DDPG ($\alpha=0$) vs. all 10 prey",r"MADDPG ($\alpha=0$) vs. all 10 prey"],fontsize = 25)
plt.legend(loc = "lower right",labels =[r"PGDDPG ($\beta=0.8$) vs. all 10 prey",r"DDPG ($\alpha=1$) vs. all 10 prey",r"MADDPG ($\alpha=5$) vs. all 10 prey" ],fontsize = 25)#20
#去掉边框
ax = plt.axes()
