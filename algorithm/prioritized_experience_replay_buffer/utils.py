...
def discount_with_dones(trajectory, gamma):
    ...
    # positive_return = 0
    normal_return = 0
    for item in trajectory[::-1]:  #trajectory[::-1] 颠倒顺序,逆序 by laker
        #item[-1]：True or False(done)
        #item[2]: reward
        r = item[2] + gamma * r * (1. - item[-1])  # fixed off by one bug
        # positive_return = (item[2] if item[2] >0 else 0) + positive_return * (1. - item[-1])
        normal_return = item[2] + normal_return * (1. - item[-1]) #sumimg all reward
        discounted.append(r) #discount reward for each step
