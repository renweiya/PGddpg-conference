...
class SoftCategoricalPd(Pd):
    ...
    def sample(self, deterministic=False):#laker denote
        # clip = 0.00000000001
        # u = tf.clip_by_value(tf.random_uniform(tf.shape(self.logits)), clip, 1 - clip)
        # u = tf.random_uniform(tf.shape(self.logits))
        ...
        if deterministic:  # TODO: ??????
            # return U.tanh(self.logits), U.tanh(self.logits - explore_direction * tf.log(-tf.log(u)))
            return U.tanh(self.logits), tf.clip_by_value(tf.random_normal(tf.shape(self.logits),U.tanh(self.logits), stddev=0.3),-1,1) 
        else:
            # return U.tanh(self.logits - explore_direction * tf.log(-tf.log(u)))
